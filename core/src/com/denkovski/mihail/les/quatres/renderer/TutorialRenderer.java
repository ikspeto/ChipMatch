...
//	private final long NANO = 1000000000;
    ...
    private int chipPositions[] = {16, 17, 23, 33};

    private PuzzleScreen puzzleScreen;
    private ActionScreen actionScreen;

    public TutorialRenderer(ChipMatch game, ScoreRenderer scoreRenderer) {
        for(int i=0;i<49;i++) {
            remaining.add(i);
        }
        this.game = game;
...
//		game.tileSize = game.pointDist-2.0f*uiRatio;
        ...
        if(game.gameType == ChipMatch.GameType.ACTION) {
...
//            }
            ...
            slots[rand].puzzleTile.qAnimation.add(-20);
            slots[rand].puzzleTile.qAxis.add(true);
            ...
            slots[rand].puzzleTile.aOrigin = rand;
            slots[rand].puzzleTile.aDest = rand;
...
//							remaining.add(i);
    ...
    private void startNextLevel() {
        // Start ZEN/ACTION appropriately
        if(game.gameType == ChipMatch.GameType.ACTION) {
//            ActionScreen actionScreen = new ActionScreen(game);
            game.setScreen(actionScreen);
        } else {
//            PuzzleScreen puzzleScreen = new PuzzleScreen(game);
